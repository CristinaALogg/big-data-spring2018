...
file_name = 'data/pset_tweets.json'
...
tweetdf = pd.read_json('/Users/cristinalogg/Desktop/github/big-data-spring2018/week-04/data/pset_tweets.json')
tweetdf.dtypes

##STEP 2
#tweetdf['location'].unique()
...
bos_list = tweets[tweets['location'].str.contains("$$$$$$$$$$")]['location'] #for where tweets have location, create a mask, where the string within the locationcon
tweets['location'].replace(bos_list, '10 Dollar Signs', inplace = True)
...
#plt.pie(df_count_tweets['count'], labels=df_count_tweets.index.get_values(), shadow=False)
plt.axis('equal')
plt.tight_layout()
plt.show()
...
tweets.to_csv('twitter_pset3_part1_3_data.csv', sep=',', encoding='utf-8')
